...
export const StylistDashboard: React.FC = () => {
  ...
  const completedTodayCount = myAssignedTokens.filter(q => q.status === 'completed').length;
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-[#635BFF] to-indigo-700 text-white p-6 rounded-3xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {(['day', 'week', 'month'] as const).map(p => (
            <button
              ...
              className={`px-3.5 py-1.5 text-xs font-bold capitalize rounded-full transition-all ${
                timePeriod === p ? 'bg-white text-purple-700 shadow-sm' : 'text-white/80 hover:text-white'
              }`}
            >
              {p}
            </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm space-y-1">
          <div className="flex items-center justify-between text-xs text-slate-500 font-semibold uppercase">
            ...
            <Scissors className="w-4 h-4 text-[#635BFF]" />
          </div>
          ...
          <p className="text-[10px] text-emerald-600 font-bold">Avg speed: {currentStylist.avgServiceTimeMins} mins</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm space-y-1">
          ...
          <p className="text-[10px] text-purple-600 font-bold">Present & Active</p>
        </div>
      </div>

      {/* My Live Customer Queue Cards */}
            ...
            myAssignedTokens.map(token => (
              <div
                ...
                className={`p-4 rounded-2xl border flex flex-col justify-between space-y-3 transition-all ${
                  token.status === 'in_service' ? 'bg-purple-50/60 border-purple-300 ring-2 ring-purple-500/20' :
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 bg-[#635BFF] text-white rounded-xl flex items-center justify-center font-extrabold text-sm shadow-xs">
                      #{token.tokenNumber}
                    </div>
                    <div>
                      <h4 className="font-extrabold text-slate-900 text-sm">{token.customerName}</h4>
                      <p className="text-xs text-purple-700 font-bold">{token.serviceNames.join(', ')}</p>
                    </div>
                  </div>

                  <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase ${
                    token.status === 'in_service' ? 'bg-purple-600 text-white' : 'bg-amber-100 text-amber-800'
                  }`}>
                  ...
                  <div className="space-x-2">
                    {token.status === 'waiting' && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#635BFF] hover:bg-[#5249E6] text-white font-bold rounded-xl text-xs shadow-xs flex items-center space-x-1 inline-flex"
                      >
                    ...
                    {token.status === 'in_service' && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs shadow-xs flex items-center space-x-1 inline-flex"
                      >
